...
inputStrMoves = ["g1f3","g8f6","c2c4","g7g6","b1c3","f8g7","d2d4","e8g8"
...
inputs = map (fromJust . readMove) inputStrMoves

inputC = [ fst tc (snd tc) | tc <- zip inputA inputB]
  where inputA = map assertEq inputStrMoves 
        inputB = map showMove inputs
