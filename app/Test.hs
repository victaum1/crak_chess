...
import Defs (author, version, name, date, mio, quit, errorCmd)
...
main_map = [
  ("quit", const quit)
  ,("help", const mainHelp)
  ,("xboard", \pa -> evalStateT eventLoop (Just True,pa)) -- xboardLoop --> eXboard
  ,("uci"   , \pa -> evalStateT eventLoop (Just False,pa)) -- mUci --> eUci
  ,("play"  , \pa -> evalStateT eventLoop (Nothing,pa))  -- mainPlay --> comPlay
  ]

-- funcs
ioSearch = mio $ putStrLn "Thinking... not really :(" -- Engine?
comXboard input = mio $ putStrLn "Not defined yet :(" -- Xbord?
comUci input = mio $ putStrLn "Not defined yet :(" -- Uci?

comPlay :: String -> StateT PlayArgs IO ()
comPlay line = do
  mio $ putStrLn "play>"
  if null line then mio $ print "" -- keep going
  else do
...
-- mThinkMove
         mio $ putStrLn "engine plays... not really :("
      else do mio $ print "" -- think -- ioSearchy
...
stop = mio $ print "" -- just keep going
...
parseArgs ["-u"] = mainLoop (setProt False)
